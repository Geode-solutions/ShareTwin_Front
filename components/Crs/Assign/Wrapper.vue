    ...
    <v-row>
      ...
      <v-col cols="1" class="pa-0">
        <v-btn
          color="primary"
          width="5"
          @click="app_store.$patch({ display_crs_assigner: false })"
...
<script setup>
  ...
  import CrsConvertButton from "@/components/Crs/Assign/Button.vue"

  const app_store = use_app_store()
  const { display_crs_assigner, object_tree_index } = storeToRefs(app_store)
  ...
  const stepper_tree = reactive({
    ...
    steps: [
      {
        step_title: "Select coordinate reference system to assign",
        component: {
