<template>
  <v-navigation-drawer
    v-model="display_georeferencing_drawer"
    clipped
    permanent
    app
    fixed
  ...
  >
    <v-row class="ma-0">
      <v-col cols="auto" class="pa-2">
        <v-btn
          text="Reset all"
          @click="reset_real_picked_points()"
          class="pa-2"
          color="primary"
        />
      </v-col>
      ...
      <v-col cols="auto">
        <v-btn
          ...
          icon="mdi-close"
          size="25"
          @click="app_store.toggle_display_georeferencing_drawer(false)"
        />
      </v-col>
    </v-row>
    <v-row class="ma-0">
      <v-card
        v-for="(real_picked_point, index) in real_picked_points"
        elevation="2"
        class="pa-0"
      >
        <v-card-title>
          <v-row align="center">
              ...
              <v-tooltip text="Toggle picking mode" location="left">
                <template v-slot:activator="{ props }">
                  <v-btn flat icon size="20" @click="pick_point(index)">
                    <v-icon size="20" color="primary" v-bind="props"
            ...
            <v-col cols="auto"> Point {{ index + 1 }} </v-col>
          </v-row>
        </v-card-title>
        <v-card-text>
          <v-row align="center">
    ...
    <v-row class="text-center pa-3 ma-0">
      <v-col cols=" 12" class="text-center px-2 py-0">
        <v-text-field
          label=" Name of the coordinate system"
          v-model="coordinate_system_name"
        />
      </v-col>
    </v-row>
...
<script setup>
  ...
  const { display_georeferencing_drawer, object_tree, object_tree_index } =
    storeToRefs(app_store)
  ...
  function disable_apply_georeferencing() {
    for (let i = 0; i < real_picked_points.length; i++) {
      if (
        [null, undefined, ""].includes(real_picked_points[i].real_x.value) ||
    ...
    if (coordinate_system_name.value !== "") {
      ...
      params.append(
        "geode_object",
        object_tree.value[object_tree_index.value].geode_object,
      )
      params.append(
        "filename",
        object_tree.value[object_tree_index.value].native_file_name,
      )
  ...
  async function apply_georeferencing() {
    ...
    params.append(
      "input_point_1_x",
      Number(real_picked_points[1].world_x.value),
    )
    params.append(
      "input_point_1_y",
      Number(real_picked_points[1].world_y.value),
    )
    params.append(
      "input_point_2_x",
      Number(real_picked_points[2].world_x.value),
    )
    params.append(
      "input_point_2_y",
      Number(real_picked_points[2].world_y.value),
    )
    ...
    params.append(
      "output_point_1_x",
      Number(real_picked_points[1].real_x.value),
    )
    params.append(
      "output_point_1_y",
      Number(real_picked_points[1].real_y.value),
    )
    params.append(
      "output_point_2_x",
      Number(real_picked_points[2].real_x.value),
    )
    params.append(
      "output_point_2_y",
      Number(real_picked_points[2].real_y.value),
    )
    ...
    await api_fetch(
      ...
      {
        response_function: (response) => {
          viewer_store.update_data({
            id: object_tree.value[object_tree_index.value].id,
          })
        },
      },
    )
