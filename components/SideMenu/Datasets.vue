    ...
    <v-row>
      <v-col cols="auto" class="pa-0 align-self-center">
        ...
        <v-btn flat icon small v-on:click.stop="app_store.toggle_object_visibility(index)">
          <v-icon>
            {{ object_tree[index].is_visible ? 'mdi-checkbox-marked-outline' : 'mdi-checkbox-blank-outline' }}
          </v-icon>
        </v-btn>
      </v-col>
      <v-col cols="auto" class="pa-0 align-self-center">
        <v-img :src="geode_objects[object_tree[index].geode_object].image" width="35" />
      </v-col>
      <v-col cols="8" class="align-self-center">
        <h4 class="overflow-x-hidden">
          {{ object_tree[index].name }}
        </h4>
      </v-col>
    </v-row>
  </v-expansion-panel-title>
